...
In a linear regression, the marginal effect of $x_1$ is constant for changing values of $x_2$ and vice versa. When an interaction term is added to the model, this is clearly not true. The next image highlights some properties of regression surfaces with interaction terms using adjustments via the packages __plotly__, __scales__, and __regress3d__.

* Using __plotly__ functions and arguments, we add scatterpoints, a hovertemplate for each point, and better labels. We adjust the colors of the scatterpoints to emphasize the low and high values of the education variable.
* The library __scales__ helps present the numeric values and percentages in the hovertext more clearly.
* __regress3d__ lets us modify the marginal effects to highlight their changing values. In `add_marginal()`, we adjust the  legend text by changing `x1_direction_name` and `x2_direction_name`. We also adjust the color of the lines using `x1_color` and `x2_color`. As before, we adjust the location of the marginal effects using `x2_constant_val` and `x1_constant_val`.
