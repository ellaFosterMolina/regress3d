...
* __regress3d__ lets us modify the marginal effects to highlight their changing values. In `add_marginals()`, we adjust the  legend text by changing `x1_direction_name` and `x2_direction_name`. We also adjust the color of the lines using `x1_color` and `x2_color`. As before, we adjust the location of the marginal effects using `x1_constant_val` and `x2_constant_val`.
