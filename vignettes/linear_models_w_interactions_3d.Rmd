...
The marginal effects are called twice in this example to highlight two key features of the regression surface. Each call to `add_marginals()` specifies different constant values for each marginal effect, using the argument `x1_constant_val` and `x2_constant_val`.
...
plot_ly( data = county_data,
  ...
  add_marginals(model = mymodel, 
                x1_constant_val = 2.5, x2_constant_val =0) %>%
  add_marginals(model = mymodel, 
                x1_constant_val = 1, x2_constant_val =1) 
```
...
Because $x_2$ (education) is binary, the magnitude of the marginal effect of $x_1$ (unemployment) when $x_2$ has the value of $0$ and $1$ is easily recoverable from the multiple regression with an interaction term. Simply estimate the regression on the subset of data where education has the value of 1, and again for 0. The standard errors in the simple regressions estimated on the subsetted data is different from those in the multiple regression, but the estimate value of the effects in the multiple regression and the subsetted simple regressions are the same. 
...
__regress3d__ lets us modify the marginal effects. In `add_marginal()`, we use the argment  legend text by changing `x1_direction_name` and `x2_direction_name`. We also adjust the color of the lines using `x1_color` and `x2_color`. As before, we adjust the location of the marginal effect using `x2_constant_val` and `x1_constant_val`.
...
These graphics correspond to the following numerical regression results. Three models are presented to allow the reader to examine three effects shown in the graphic.
...
2.  A simple regression using only logged unemployment rate and counties **below** the national mean education level (low education) for the marginal effect shown with a [yellow]{style="color: yellow;"} line.
3.  A simple regression using only logged unemployment rate and counties **above** the national mean education level (high education) for the marginal effect shown with an [orange]{style="color: orange;"} line.
