...
Because $x_2$ (education) is binary, the magnitude of the marginal effect of $x_1$ (unemployment) when $x_2$ has the value of $0$ and $1$ is easily recoverable from the multiple regression with an interaction term. Simply estimate the regression on the subset of data where education has the value of 1, and again for 0. 

* The standard errors for unemployment in the simple regression estimated for `college_2cat_num` $== 0$  are **different** from those created in the multiple regression where `college_2cat_num` $== 0$. The same holds for `college_2cat_num` $== 1$.
* The estimated value for unemployment in the simple regression estimated for `college_2cat_num` $== 0$  are **the same** as those created in the multiple regression where `college_2cat_num` $== 0$. The same holds for `college_2cat_num` $== 1$.
...
* __regress3d__ lets us modify the marginal effects. In `add_marginal()`, we adjust the  legend text by changing `x1_direction_name` and `x2_direction_name`. We also adjust the color of the lines using `x1_color` and `x2_color`. As before, we adjust the location of the marginal effects using `x2_constant_val` and `x1_constant_val`.
...
plot_ly(data = county_data, 
  ...
  add_markers(size = ~pop_estimate16,
              hovertemplate = ~paste("<b>", county_state,"</b><br>",
                            ...
                            "<br><br>", 
                            "County unemployment (logged): ",round(prcnt_unemployed_log, 2),"<br>",
